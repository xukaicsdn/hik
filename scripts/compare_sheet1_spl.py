...
# 用户可直接在这里改参数（代码内指定）
# =========================
SELF_FILE_CONFIG = r"E:\work\hik\ease5_auto\【大型教室】【单声源，三分之一倍频程，A加权，直达声压】【1000HZ】- 声压数据_20260422_170629.xlsx"
EASE5_FILE_CONFIG = r"E:\work\hik\ease5_auto\【大型教室】【单声源，三分之一倍频程，A加权，直达声压】【1000HZ】- ES5数据_20260422_120334.xlsx"
...
def match_by_ease5_row_nearest_column(
    ...
    self_layout = build_row_col_layout(df_self, col_count=col_count)
    ease_layout = build_row_col_layout(df_ease5, col_count=col_count)

    rows = []
    max_self_row = int(self_layout["行号"].max()) if len(self_layout) else 0
    for _, e in ease_layout.iterrows():
        ease_row = int(e["行号"])
        if ease_row <= max_self_row:
            candidates = self_layout[self_layout["行号"] == ease_row]
        else:
            candidates = self_layout

        if len(candidates) == 0:
            continue

        diff = candidates[["x", "y", "z"]].to_numpy(dtype=float) - e[["x", "y", "z"]].to_numpy(dtype=float)
        dists = np.linalg.norm(diff, axis=1)
        best_idx = int(np.argmin(dists))
        s = candidates.iloc[best_idx]
        ...
        rows.append(
            {
                ...
                "self_y": float(s["y"]),
                "self_z": float(s["z"]),
                "self_spl": float(s["spl"]),
                "delta_db(self-ease5)": float(s["spl"] - e["spl"]),
                "match_dist_xyz": dist,
            }
        )
...
def main() -> None:
    ...
    df_self = read_sheet(self_path, self_sheet)
    df_ease5 = read_sheet(ease5_path, ease5_sheet)
